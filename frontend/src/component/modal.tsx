...
export const CreateTeamModal = ({closeModal, teamName, setTeamName, handleCreateTeam}: ModalProps) => {
	return (
		<S.ModalContainer>
			<S.ModalContent>
				...
				<S.ButtonGroup>
					...
					<Button text={"만들기"} onClick={handleCreateTeam} buttonColor="black"/>
		...
		<S.ModalContainer>
			<S.InviteContent invitedMembersCount={invitedMembers.length}>
				...
				<S.InviteInput>
					...
					/>
					<Button text="초대" onClick={handleAddAndUpdate} buttonColor="black"/>
				</S.InviteInput>


				<S.InvitedMembersList>
					{invitedMembers.map((invitedMember, index) => (
						<S.InvitedMember key={index}>{invitedMember}
							<Trash/></S.InvitedMember>
					))}
				</S.InvitedMembersList>
				<S.ButtonGroup>
					<Button text="취소" onClick={closeModal}/>
